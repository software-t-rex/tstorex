...
describe('createStore', () => {
	...
	describe('store.getScopeStore', () => {
		...
		it("sould notify another scoped store with same path on change", () => {
			const listener = vi.fn()
			const listener2 = vi.fn()
			const store = createStore<typeof testState2 & {pet?:{name:string}}>(testState2)
			const scopeStore = store.getScopeStore('mum.fullname')
			const scopeStore2 = store.getScopeStore('mum.fullname')
			scopeStore.subscribe(listener)
			scopeStore2.subscribe(listener2)
			scopeStore.set("Jane Black")
			expect(listener).toHaveBeenCalledTimes(1)
			expect(listener2).toHaveBeenCalledTimes(1)
			scopeStore2.set("Axel Roses")
			expect(listener).toHaveBeenCalledTimes(2)
			expect(listener2).toHaveBeenCalledTimes(2)
			expect(scopeStore.get()).toStrictEqual("Axel Roses")
			store.set((s) => ({...s, mum: {...s.mum, fullname: 'Jane White'}}))
			expect(listener).toHaveBeenCalledTimes(3)
			expect(listener2).toHaveBeenCalledTimes(3)
			expect(scopeStore.get()).toStrictEqual(scopeStore2.get())
		})
